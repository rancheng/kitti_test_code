...
class AddPointCloudTimerCallback():
    def __init__(self, renderer, lidar_iter, dsize):
        self.lidar_iter = lidar_iter
        self.renderer = renderer
        self.iterations = 0
        self.dsize = dsize

    def execute(self, iren, event):
        if self.iterations < self.dsize:
            pointCloud = VtkPointCloud()
            self.renderer.AddActor(pointCloud.vtkActor)
            pointCloud.clearPoints()
            points = next(self.lidar_iter)
            for p in points:
                pointCloud.addPoint(p[0:3])
...
def render_kitti_velo(velo_iter, dsize):
    # Renderer
    ...
    addPointCloudTimerCallback = AddPointCloudTimerCallback(renderer, velo_iter, dsize)
